#!/usr/bin/env python3
"""Compile all 1,806 high-frequency Taylor panels into bounded Lean batches.
...
def main():
    ...
    all_keys = [(i, j) for i, r in enumerate(rows) for j, p in enumerate(r['panel_data'])
                if p['method'] == 'taylor' and p['part'] == 'high']
    if len(all_keys) != 1806:
        raise ValueError('High-frequency coverage differs')
    keys = all_keys[:args.limit] if args.limit is not None else all_keys
    destination = root / 'BerryEsseen/Certificates/FiniteHighTaylorPanels'
    destination.mkdir(parents=True, exist_ok=True)
    prefix = 'FiniteHighTaylorPanel'
    with tempfile.TemporaryDirectory(prefix='finite_high_') as temporary:
        temp = Path(temporary)
        def emit(key):
            i, j = key
            out = temp / f'Panel{i}_{j}.lean'
            p = subprocess.run([sys.executable, str(root / 'scripts/generate_high_panel.py'),
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            for future in as_completed([pool.submit(emit, key) for key in keys]):
                ...
                if completed % 100 == 0 or completed == len(keys):
                    print(f'{completed}/{len(keys)} high-frequency proofs generated', flush=True)
        files, entries = [], []
        for start in range(0, len(keys), args.batch_size):
            ...
            for i, j in batch:
                ...
                entries.append({'row_position': i, 'panel_index': j,
                                ...
                                'kernel_region': rows[i]['panel_data'][j]['kernel_region']})
            content = (header + '\n'.join(bodies)).encode()
    ...
    lines = ['module', '']
    lines += [f'public import BerryEsseen.Certificates.FiniteHighTaylorPanels.{Path(f["name"]).stem}' for f in files]
    lines += ['', '@[expose] public section', '', 'set_option maxRecDepth 1000000',
              'set_option maxHeartbeats 0', '', 'namespace FiniteHighTaylorPanels', '',
    ...
    manifest = {'schema': 'finite-high-taylor-panels-v1', 'scope': __doc__,
