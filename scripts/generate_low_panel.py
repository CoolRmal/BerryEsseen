#!/usr/bin/env python3
"""Generate a low-frequency modulus Taylor panel with full function semantics.
...
Lean. It is restricted to saved low-frequency modulus panels.
"""
...
def evaluate(nodes):
    ...
    for node in nodes:
        ...
        else: raise ValueError(f"Unexpected low-panel operation {op}")
        values.append(out)
    return values
...
def generate(row_index, panel_index, output, namespace_prefix="FiniteLowPanel"):
    ...
    if (panel["part"], panel["method"]) != ("low", "taylor") or panel["magnitude_branch"] not in {"line", "anchor-pos-a"}:
        raise ValueError("This compiler handles saved low-frequency Taylor panels only")
    ...
    Series.start_dag()
    mid = cell.function("low", Series([I(m), I(h)], 5), *selectors)
    whole = cell.function("low", Series([I(l).union(I(r)), I(h)], 5), *selectors)
    roots = [mid.dag_node(), whole.dag_node()]
    nodes = Series.stop_dag()
    vals = evaluate(nodes)
    cache = {}
    used_constants = {}
    needed_nodes = set(ancestors(nodes, roots))
    for i, node in enumerate(nodes):
        if i in needed_nodes and node["op"] == "constant":
    ...
    needed_nodes = set(ancestors(nodes, roots))
    for i, node in enumerate(nodes):
        if i not in needed_nodes or node["op"] != "refine0":
            continue
    ...
    if half not in {"left", "right"} or (half == "left" and panel["magnitude_branch"] == "cos"):
        raise ValueError("Unsupported low-frequency kernel region")
    integrand = "finiteHighLeftIntegrand" if half == "left" else "finiteHighRightIntegrand"
    anchored = n > 1 and panel["magnitude_branch"].startswith("anchor-")
    sigma = "1" if "pos" in panel["magnitude_branch"] else "(-1)"
    if n == 1:
        modulus_term = "fun _ ↦ (1 : ℝ)"
    elif anchored:
    ...
    nhi = row["n_hi"]
    error_branch = panel["error_branch"]
    if error_branch not in {"anchored", "classical"}:
        raise ValueError("Unsupported low-frequency error branch")
    integrand_target = f"finiteLowIntegrand {n} {nhi} {c} ({modulus_term}) (finiteErrorMajorant .{error_branch} {beta})"
    ...
    unfolded += ["finiteLowIntegrand", "finiteErrorMajorant", "finiteAnchorError", "finiteClassicalError", "gaussianCosineQ", "Finset.sum_range_succ", "Finset.sum_range_zero", "finiteHighRightIntegrand", "finiteHighLeftIntegrand", "finiteRightKernel", "finiteScaledKernel", "finiteCotangentLower", "finiteLineModulus", "finiteCosineModulus", "finiteAnchorModulus", "finiteImaginaryBound", "minorantScale", "div_mul_eq_div_div", "finiteCotangentUpper",
    ...
    lines += [f"theorem whole_function_eq (t : ℝ) : f{roots[1]} t =",
              f"    {integrand_target} ((center : ℝ) + (radius : ℝ) * t) := by",
    ...
    lines += [f"def upper : Rat := {qlit(budget)}", "",
              ...
              f"    {integrand_target} s) ≤ (upper : ℝ) := by",
              f"  have he : f{roots[1]} = (fun t ↦ {integrand_target}",
              ...
              f"theorem actual_low_integral {{μ : ProbabilityMeasure ℝ}} (hμ : Admissible μ)",
              f"    {{n : ℕ}} (hn : {n} ≤ n) (hnH : n ≤ {nhi}) (hβ : thirdAbsMoment μ ≤ {beta}) :",
              f"    (∫ s in ({qlit(l)} : ℝ)..{qlit(r)}, prawitzLowError",
              f"      (normalizedSumLaw μ n) ({c} * sqrt n) s) ≤ (upper : ℝ) := by"]
    pi_guard = "(by have hp := FiniteDarbouxCertificates.piLower_le_pi; norm_num [FiniteDarbouxCertificates.piLower] at hp ⊢; linarith)"
    if n == 1:
        lines += ["  apply finite_low_panel_bound hμ (by norm_num) hn hnH hβ (by norm_num)",
                  f"    (by norm_num) (by norm_num) (by norm_num) {pi_guard}",
                  f"    (fun _ ↦ 1) continuous_const .{error_branch}",
                  "    (fun s hs ↦ modulusBound_le_one _ _) (fun s hs ↦ le_rfl) envelope_integral"]
    elif anchored:
        lines += ["  apply finite_low_anchor_panel_bound hμ (by norm_num) hn hnH hβ (by norm_num)",
                  f"    (by norm_num) (by norm_num) (by norm_num) {pi_guard} .cubic .{error_branch}",
                  "    (fun s hs ↦ cosine_sign hs) (fun s hs ↦ square_le_one hs) envelope_integral"]
    else:
        lines += ["  exact finite_low_line_panel_bound hμ (by norm_num) hn hnH hβ (by norm_num)",
                  f"    (by norm_num) (by norm_num) (by norm_num) {pi_guard} (by norm_num) .{error_branch}",
                  "    envelope_integral"]
    lines += ["", "def certificate : CertifiedLowPanel :=",
              f"  ⟨{n}, {nhi}, {beta}, {c}, {qlit(l)}, {qlit(r)}, upper, by",
...
def main():
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "BerryEsseen/Certificates/FiniteLowPanels/Panel0_3.lean")
    parser.add_argument("--namespace-prefix", default="FiniteLowPanel")
