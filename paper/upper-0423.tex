...
\item We claim no independent verification. In particular, the repository's
  ...
  targets the \emph{separate} $0.4688$ theorem; we have not run any comparator on
  the $0.423$ theorem, and a configuration allowing only the three standard
  axioms would reject it.
...
\end{leancode}
The default library target does not import \file{Upper0423}, so the existing
continuous integration and the comparator setup for the separate $0.4688$
theorem are unaffected. A full build is expensive because it evaluates all $579$
certificates.
...
\paragraph{The separate $0.4688$ theorem.} The same repository contains an
independent development of the weaker bound $C_{\mathrm{BE}}\le0.4688$ which
avoids \Lean{native\_decide} altogether and is therefore checkable with only the
three standard axioms; the trusted comparator configuration in
\file{comparator/} targets that theorem. The $0.423$ development described here
is disjoint from it: it neither imports nor modifies any of its modules.
