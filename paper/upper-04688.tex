...
\item \textbf{Comparator: pending.} The final pinned, sandboxed Linux
  ...
  verification.
\item \textbf{Local build.} The repository's own documentation records that
  \texttt{lake build Solution BerryEsseen} has passed. Independently, on the
  machine used to prepare this document, the build tree
  \file{.lake/build/lib/lean/BerryEsseen/} contained $1{,}272$ compiled
  \file{.olean} modules at the time of writing, and no \file{Solution.olean} or
  \file{Challenge.olean} was present; a build was in progress. We therefore
  make no independent claim about a completed local build here, only the two
  observations just stated.
\end{itemize}
